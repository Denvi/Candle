...
{
    if ((value >= QHistoryState::ShallowHistory) && (value <= QHistoryState::DeepHistory))
        return qtscript_QHistoryState_HistoryType_keys[static_cast<int>(value)-static_cast<int>(QHistoryState::ShallowHistory)];
    return QString();
}
...
static QScriptValue qtscript_construct_QHistoryState_HistoryType(QScriptContext *context, QScriptEngine *engine)
{
    ...
    if ((arg >= QHistoryState::ShallowHistory) && (arg <= QHistoryState::DeepHistory))
