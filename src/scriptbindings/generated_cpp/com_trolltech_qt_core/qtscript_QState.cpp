...
{
    if ((value >= QState::ExclusiveStates) && (value <= QState::ParallelStates))
        return qtscript_QState_ChildMode_keys[static_cast<int>(value)-static_cast<int>(QState::ExclusiveStates)];
    return QString();
}
...
static QScriptValue qtscript_construct_QState_ChildMode(QScriptContext *context, QScriptEngine *engine)
{
    ...
    if ((arg >= QState::ExclusiveStates) && (arg <= QState::ParallelStates))
