...
static QScriptValue qtscript_QGLFramebufferObjectFormat_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    case 1:
    if (context->argumentCount() == 0) {
        unsigned int _q_result = _q_self->internalTextureFormat();
    ...
    case 11:
    if (context->argumentCount() == 0) {
        unsigned int _q_result = _q_self->textureTarget();
        return QScriptValue(context->engine(), _q_result);
    }
    break;
...
QScriptValue qtscript_create_QGLFramebufferObjectFormat_class(QScriptEngine *engine)
{
    ...
    QScriptValue proto = engine->newVariant(QVariant::fromValue((QGLFramebufferObjectFormat*)0));
    for (int i = 0; i < 13; ++i) {
