...
{
    if ((value >= QLocalSocket::UnconnectedState) && (value <= QLocalSocket::ClosingState))
        return qtscript_QLocalSocket_LocalSocketState_keys[static_cast<int>(value)-static_cast<int>(QLocalSocket::UnconnectedState)];
    return QString();
}
...
static QScriptValue qtscript_construct_QLocalSocket_LocalSocketState(QScriptContext *context, QScriptEngine *engine)
{
    ...
    if ((arg >= QLocalSocket::UnconnectedState) && (arg <= QLocalSocket::ClosingState))
...
{
    if ((value >= QLocalSocket::ConnectionRefusedError) && (value <= QLocalSocket::OperationError))
        return qtscript_QLocalSocket_LocalSocketError_keys[static_cast<int>(value)-static_cast<int>(QLocalSocket::ConnectionRefusedError)];
    return QString();
}
...
{
    int arg = context->argument(0).toInt32();
    if ((arg >= QLocalSocket::ConnectionRefusedError) && (arg <= QLocalSocket::OperationError))
