...
static QScriptValue qtscript_QPolygon_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    case 5:
    if (context->argumentCount() == 0) {
        const QPoint& _q_result = _q_self->constFirst();
        return qScriptValueFromValue(context->engine(), _q_result);
    }
    break;
    ...
    case 6:
    if (context->argumentCount() == 0) {
        const QPoint& _q_result = _q_self->constLast();
        return qScriptValueFromValue(context->engine(), _q_result);
    }
    break;
    ...
    if (context->argumentCount() == 1) {
        QPolygon _q_arg0 = qscriptvalue_cast<QPolygon>(context->argument(0));
        QPolygon& _q_result = _q_self->operator=(_q_arg0);
        return qScriptValueFromValue(context->engine(), _q_result);
    }
    break;
...
QScriptValue qtscript_create_QPolygon_class(QScriptEngine *engine)
{
    ...
    QScriptValue proto = engine->newVariant(QVariant::fromValue((QPolygon*)0));
    for (int i = 0; i < 54; ++i) {
