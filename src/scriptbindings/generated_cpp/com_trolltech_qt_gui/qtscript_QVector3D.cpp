...
static const char * const qtscript_QVector3D_function_signatures[] = {
    ...
    , "int i\nint i"
    , "QVector3D vector"
    , "QMatrix4x4 modelView, QMatrix4x4 projection, QRect viewport"
    , "float x"
    , "float y"
    , "float z"
    , ""
...
static QScriptValue qtscript_QVector3D_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (context->argumentCount() == 3) {
        ...
        QVector3D _q_result = _q_self->project(_q_arg0, _q_arg1, _q_arg2);
        return qScriptValueFromValue(context->engine(), _q_result);
    }
    break;
    ...
    if (context->argumentCount() == 3) {
        ...
        QMatrix4x4 _q_arg1 = qscriptvalue_cast<QMatrix4x4>(context->argument(1));
        QRect _q_arg2 = qscriptvalue_cast<QRect>(context->argument(2));
        QVector3D _q_result = _q_self->unproject(_q_arg0, _q_arg1, _q_arg2);
        return qScriptValueFromValue(context->engine(), _q_result);
    }
    break;
...
QScriptValue qtscript_create_QVector3D_class(QScriptEngine *engine)
{
    ...
    QScriptValue proto = engine->newVariant(QVariant::fromValue((QVector3D*)0));
    for (int i = 0; i < 27; ++i) {
