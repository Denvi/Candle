...
{
    if ((value >= QWizard::ClassicStyle) && (value <= QWizard::NStyles))
        return qtscript_QWizard_WizardStyle_keys[static_cast<int>(value)-static_cast<int>(QWizard::ClassicStyle)];
    return QString();
}
...
{
    int arg = context->argument(0).toInt32();
    if ((arg >= QWizard::ClassicStyle) && (arg <= QWizard::NStyles))
