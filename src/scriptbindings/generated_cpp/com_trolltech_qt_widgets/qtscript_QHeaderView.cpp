...
{
    if ((value >= QHeaderView::Interactive) && (value <= QHeaderView::ResizeToContents))
        return qtscript_QHeaderView_ResizeMode_keys[static_cast<int>(value)-static_cast<int>(QHeaderView::Interactive)];
    return QString();
}
...
static QScriptValue qtscript_construct_QHeaderView_ResizeMode(QScriptContext *context, QScriptEngine *engine)
{
    ...
    if ((arg >= QHeaderView::Interactive) && (arg <= QHeaderView::ResizeToContents))
...
static QScriptValue qtscript_QHeaderView_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    case 10:
    if (context->argumentCount() == 0) {
        _q_self->resetDefaultSectionSize();
        return context->engine()->undefinedValue();
    }
    break;
