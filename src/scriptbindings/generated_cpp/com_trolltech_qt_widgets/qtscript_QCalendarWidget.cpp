...
{
    if ((value >= QCalendarWidget::NoHorizontalHeader) && (value <= QCalendarWidget::LongDayNames))
        return qtscript_QCalendarWidget_HorizontalHeaderFormat_keys[static_cast<int>(value)-static_cast<int>(QCalendarWidget::NoHorizontalHeader)];
    return QString();
}
...
static QScriptValue qtscript_construct_QCalendarWidget_HorizontalHeaderFormat(QScriptContext *context, QScriptEngine *engine)
{
    ...
    if ((arg >= QCalendarWidget::NoHorizontalHeader) && (arg <= QCalendarWidget::LongDayNames))
