...
{
    if ((value >= QLCDNumber::Hex) && (value <= QLCDNumber::Bin))
        return qtscript_QLCDNumber_Mode_keys[static_cast<int>(value)-static_cast<int>(QLCDNumber::Hex)];
    return QString();
}
...
static QScriptValue qtscript_construct_QLCDNumber_Mode(QScriptContext *context, QScriptEngine *engine)
{
    ...
    if ((arg >= QLCDNumber::Hex) && (arg <= QLCDNumber::Bin))
