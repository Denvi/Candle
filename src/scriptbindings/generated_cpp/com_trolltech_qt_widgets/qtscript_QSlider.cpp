...
{
    if ((value >= QSlider::NoTicks) && (value <= QSlider::TicksBothSides))
        return qtscript_QSlider_TickPosition_keys[static_cast<int>(value)-static_cast<int>(QSlider::NoTicks)];
    return QString();
}
...
static QScriptValue qtscript_construct_QSlider_TickPosition(QScriptContext *context, QScriptEngine *engine)
{
    ...
    if ((arg >= QSlider::NoTicks) && (arg <= QSlider::TicksBothSides))
