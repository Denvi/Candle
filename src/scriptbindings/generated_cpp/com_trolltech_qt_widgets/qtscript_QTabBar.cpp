...
{
    if ((value >= QTabBar::RoundedNorth) && (value <= QTabBar::TriangularEast))
        return qtscript_QTabBar_Shape_keys[static_cast<int>(value)-static_cast<int>(QTabBar::RoundedNorth)];
    return QString();
}
...
{
    int arg = context->argument(0).toInt32();
    if ((arg >= QTabBar::RoundedNorth) && (arg <= QTabBar::TriangularEast))
