...
{
    if ((value >= QStackedLayout::StackOne) && (value <= QStackedLayout::StackAll))
        return qtscript_QStackedLayout_StackingMode_keys[static_cast<int>(value)-static_cast<int>(QStackedLayout::StackOne)];
    return QString();
}
...
static QScriptValue qtscript_construct_QStackedLayout_StackingMode(QScriptContext *context, QScriptEngine *engine)
{
    ...
    if ((arg >= QStackedLayout::StackOne) && (arg <= QStackedLayout::StackAll))
