...
public:
    ...
    static QList<QByteArray> splitCommand(const QString &command);
    static double parseCoord(const QStringList &args, char c);
    ...
    static QVector3D updateCenterWithCommand(const QStringList &commandArgs, const QVector3D &initial, const QVector3D &nextPoint, bool absoluteIJKMode, bool clockwise);
    static QString generateG1FromPoints(const QVector3D &start, const QVector3D &end, bool absoluteMode, int precision);
    static double getAngle(const QVector3D &start, const QVector3D &end);
