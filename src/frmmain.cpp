...
frmMain::frmMain(QWidget* parent)
    : QMainWindow(parent)
    , ui(new Ui::frmMain)
    , m_status { QStringLiteral("Unknown"), QStringLiteral("Idle"), QStringLiteral("Alarm"), QStringLiteral("Run"), QStringLiteral("Home"), QStringLiteral("Hold:0"), QStringLiteral("Hold:1"), QStringLiteral("Queue"), QStringLiteral("Check"), QStringLiteral("Door"), QStringLiteral("Jog") } // TODO: Update "Door" state
    , m_statusCaptions { tr("Unknown"), tr("Idle"), tr("Alarm"), tr("Run"), tr("Home"), tr("Hold"), tr("Hold"), tr("Queue"), tr("Check"), tr("Door"), tr("Jog") }
    , m_statusBackColors { QStringLiteral("red"), QStringLiteral("palette(button)"), QStringLiteral("red"), QStringLiteral("lime"), QStringLiteral("lime"), QStringLiteral("yellow"), QStringLiteral("yellow"), QStringLiteral("yellow"), QStringLiteral("palette(button)"), QStringLiteral("red"), QStringLiteral("lime") }
    , m_statusForeColors { QStringLiteral("white"), QStringLiteral("palette(text)"), QStringLiteral("white"), QStringLiteral("black"), QStringLiteral("black"), QStringLiteral("black"), QStringLiteral("black"), QStringLiteral("black"), QStringLiteral("palette(text)"), QStringLiteral("white"), QStringLiteral("black") }
{
...
void frmMain::saveSettings()
{
    ...
    if (ui->scrollContentsDevice->isVisible())
        panels << ui->scrollContentsDevice->saveState();
    if (ui->scrollContentsModification->isVisible())
        panels << ui->scrollContentsModification->saveState();
    if (ui->scrollContentsUser->isVisible())
        panels << ui->scrollContentsUser->saveState();
    ...
    while (m_serialPort.canReadLine()) {
        ...
        if (data[0] == '<') {
            ...
            if (mpx.indexIn(data) != -1) {
                ...
                // Update stored vars
                m_storedVars.setCoords("M", QVector3D(ui->txtMPosX->value(), ui->txtMPosY->value(), ui->txtMPosZ->value()));
            }
            ...
            // Update stored vars
            m_storedVars.setCoords("W", QVector3D(ui->txtWPosX->value(), ui->txtWPosY->value(), ui->txtWPosZ->value()));
            ...
            if (m_processingFile && status != CHECK) {
                ...
                for (int i = m_lastDrawnLineIndex; i < list.count()
                     && list.at(i)->getLineNumber()
                         <= (m_currentModel->data(m_currentModel->index(m_fileProcessedCommandIndex, 4)).toInt() + 1);
                     i++) {
                ...
                if ((m_commands[0].command != "[CTRL+X]" && dataIsEnd(data))
                    ...
                    if (ca.command.contains("G38.2") && ca.tableIndex < 0) {
                        ...
                        if (PRB.indexIn(response) != -1) {
                            m_storedVars.setCoords("PRB", QVector3D(PRB.cap(1).toDouble(), PRB.cap(2).toDouble(), PRB.cap(3).toDouble()));
                        }
                    }
                    ...
                    if (m_queue.length() > 0) {
                        ...
                        while ((bufferLength() + cq.command.length() + 1) <= BUFFERLENGTH) {
                            ...
                            if (!cq.command.isEmpty() && (m_queue.isEmpty() || cq.queue))
                                break;
                            else
...
QTime frmMain::updateProgramEstimatedTime(QList<LineSegment*> lines)
{
    ...
    for (int i = 0; i < lines.count(); i++) {
        ...
        if (!qIsNaN(length) && !qIsNaN(ls->getSpeed()) && ls->getSpeed() != 0)
            time += length / ((ui->slbFeedOverride->isChecked() && !ls->isFastTraverse()) ? (ls->getSpeed() * ui->slbFeedOverride->value() / 100) : (ui->slbRapidOverride->isChecked() && ls->isFastTraverse()) ? (ls->getSpeed() * ui->slbRapidOverride->value() / 100) : ls->getSpeed());
    }
...
void frmMain::restoreOffsets()
{
    ...
    sendCommand(QString("%4G53G90X%1Y%2Z%3").arg(ui->txtMPosX->value()).arg(ui->txtMPosY->value()).arg(ui->txtMPosZ->value()).arg(m_settings->units() ? "G20" : "G21"),
        -2, m_settings->showUICommands());

    sendCommand(QString("%4G92X%1Y%2Z%3").arg(ui->txtWPosX->value()).arg(ui->txtWPosY->value()).arg(ui->txtWPosZ->value()).arg(m_settings->units() ? "G20" : "G21"),
        -2, m_settings->showUICommands());
}
...
void frmMain::storeOffsetsVars(QString response)
{
    ...
    while ((p = gx.indexIn(response, p)) != -1) {
        m_storedVars.setCoords(gx.cap(1), QVector3D(gx.cap(2).toDouble(), gx.cap(3).toDouble(), gx.cap(4).toDouble()));

        p += gx.matchedLength();
    }

    if (tx.indexIn(response) != -1) {
        m_storedVars.setCoords(tx.cap(1), QVector3D(0, 0, tx.cap(2).toDouble()));
    }
}
...
void frmMain::on_actServiceSettings_triggered()
{
    ...
    if (m_settings->exec()) {
        if (m_settings->port() != "" && (m_settings->port() != m_serialPort.portName() || m_settings->baud() != m_serialPort.baudRate())) {
            if (m_serialPort.isOpen())
                m_serialPort.close();
            m_serialPort.setPortName(m_settings->port());
            m_serialPort.setBaudRate(m_settings->baud());
...
{
    // Restore offset
    sendCommand(QString("%4G53G90G0X%1Y%2Z%3").arg(ui->txtMPosX->value()).arg(ui->txtMPosY->value()).arg(ui->txtMPosZ->value()).arg(m_settings->units() ? "G20" : "G21"),
        -2, m_settings->showUICommands());
...
void frmMain::on_grpOverriding_toggled(bool checked)
{
    if (checked) {
        ...
        ui->grpOverriding->setTitle(tr("Overriding") + QString(tr(" (%1/%2/%3)")).arg(ui->slbFeedOverride->isChecked() ? QString::number(ui->slbFeedOverride->value()) : "-").arg(ui->slbRapidOverride->isChecked() ? QString::number(ui->slbRapidOverride->value()) : "-").arg(ui->slbSpindleOverride->isChecked() ? QString::number(ui->slbSpindleOverride->value()) : "-"));
    }
    updateLayouts();
                            ...
                            if (list->at(j)->getLineNumber() == line) {
                                if (!qIsNaN(list->at(j)->getEnd().length()) && (isLinearMove || (!hasCommand && !lastCode.isEmpty()))) {
                                    ...
                                    while ((j < list->count()) && (list->at(j)->getLineNumber() == line)) {
                                        ...
                                        item.command = newCommand + QString("X%1Y%2Z%3").arg(point.x(), 0, 'f', 3).arg(point.y(), 0, 'f', 3).arg(point.z(), 0, 'f', 3);
                                        m_programHeightmapModel.data().append(item);
